...
# import functions from cohort utilities 
import COVID19_Maternity_Inpatient_Anticoagulant.A_get_cohorts.cohort_utilities


# Workflow of A_generate_cohort1.py 
# 1. Get source population - deliveries from 03-05-2020 ~ 10-20-2022
# 2. Exclude patients based on inclusion/exclusion criteria - Exclude multiple gestation, age < 18, age >= 45, gestational days < 140 
# 3. Get COVID-19 records before and during pregnancy  
# 3.1 Get COVID-19 patients based on test record 
# 3.2 Get COVID-19 patients based on diagnosis 
# 3.3 COVID-19 before pregnancy - for feature to use later in the analysis 
# 4. Get Vaccination record - fore feature to use later in the analysis  
# 5. Get anticoagulant administration record
...
# get patients who were covid patients before pregnancy - based on diagnosis
cc_list = ['covid_19']
...
# get patients who were covid infected before pregnancy - based on test result 
...
#patients who were vaccinated after the delivery
...
#patients who were not vaccinated at all
...
# total unvaccinated patients before delivery 
...
import datetime
DATE_CUTOFF = datetime.datetime(2022, 10, 20, 0, 0, 0, 0) # date cutoff
df_cohort_covid_maternity = spark.sql("SELECT * FROM rdp_phi_sandbox.snp2_cohort_covid_maternity_expanded_5").na.drop(subset=['pat_id', 'episode_id', 'child_episode_id']) #load PCR test result
df_cohort_covid_maternity = df_cohort_covid_maternity.na.drop(subset=["gestational_days"]).distinct()

# covid-19 patients based on the test 
df_cohort_covid_maternity = determine_covid_maternity_cohort(df_cohort_covid_maternity, test_result='positive').sort_index()

# get covid patients based on diagnoses
cc_list = ['covid_19']
problemlist_covid = get_problem_list(df_cohort_maternity_filtered.drop('name'), add_cc_columns = cc_list)
encounter_covid = get_encounters(df_cohort_maternity_filtered.drop('name'), add_cc_columns = cc_list, join_table= 'encounterdiagnosis')
...
# load patients who had covid infection during pregnancy 
...
# combine patients who had covid infection during and before 
...
# generate feature column indicating prior covid infection before 
...
print('# Number of unique patients with history of COVID-19 infection during pregnancy: ' + str(covid19_pregnancy_ag.count()))


# get anticoagulant administration during pregnancy + -14 ~ +60 from the COVID-19 infection 
...
medication_admin_aggregated_df = medication_admin_aggregated_df.withColumn("med_administration_date", to_date(col("time_index_day"),"yyyy-MM-dd"))
medication_admin_aggregated_df.createOrReplaceTempView("meds")
# patients who had no civid but anticoagulant 
...
# filter for patients who had valid anticoagulant administration 
...
# limit to patients who had prophylactic dossage
